...
def build_pdf_url(attachment_name):
    ...
    if attachment_name.endswith('.pdf'):
        ...
        return PDF_VIEW_BASE + attachment_name + '.pdf'
...
if fetch_btn:
    ...
    if table is not None:
        ...
        cols = [
            "DT_TM", "SLONGNAME", "HEADLINE", "SUBCATNAME", "CRITICALNEWS",
            "ATTACHMENTNAME"
        ]
        ...
        # Prepare data for AgGrid with clickable ATTACHMENTNAME
        df_aggrid = df_display.reset_index(drop=True).copy()
        if "ATTACHMENTNAME" in df_aggrid.columns:
            df_aggrid["ATTACHMENTNAME"] = df_aggrid.apply(lambda row: f"[{row['ATTACHMENTNAME']}]({build_pdf_url(row['ATTACHMENTNAME'])})" if row['ATTACHMENTNAME'] else '', axis=1)

        # AgGrid with filters
        gb = GridOptionsBuilder.from_dataframe(df_aggrid)
        gb.configure_default_column(filter=True, sortable=True)
        if "ATTACHMENTNAME" in df_aggrid.columns:
            gb.configure_column("ATTACHMENTNAME", cellRenderer="markdown")
        gridOptions = gb.build()

        AgGrid(df_aggrid, gridOptions=gridOptions, enable_enterprise_modules=False, fit_columns_on_grid_load=True, reload_data=False)
